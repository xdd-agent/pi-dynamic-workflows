...
function resolveWriteTaskIdentity(label, prompt) {
    return resolveIdentity(label, prompt, ["alpha", "beta"]);
}
function resolveIdentity(label, prompt, identities) {
...
function resolveExclusiveIdentity(label, prompt, identities) {
    const labelIdentities = identities.filter((identity) => new RegExp(`\\b${identity}\\b`, "i").test(label));
    if (labelIdentities.length === 1)
        return labelIdentities[0];
...
function resolveGenerateFilterIdentity(label, prompt, identities) {
...
const fullWriteAgentFixture = (request) => {
...
const SCENARIO_AGENT_FIXTURES = {
    ...
    "coverage-generate-filter": (request, state) => {
        if (isGenerateFilterGeneratorSchema(request.schema)) {
            state.attempt++;
            const candidates = state.attempt === 1
                ? [
...
export function selectComprehensionScenarios(suite) {
...
export async function runComprehensionScenario(options) {
...
function normalizeModelSelection(options) {
...
function assertScenario(scenario, skillLoadingEvidence, runtime) {
    const assertions = [];
    assertions.push({
        name: "skill:loaded",
    ...
    if (scenario.id === "quick-write") {
        ...
        assertions.push({
            ...
            passed: taskCalls.every((call) => call !== undefined && call.label.trim() !== "") &&
    ...
    if (scenario.id === "full-write") {
        ...
        assertions.push({
            ...
            passed: alphaCall !== undefined &&
                containsInNamedField(runtime.result, /complet|success|done/i, (value) => containsTaskResultEvidence(value, "alpha", alphaCall.result)),
            details: "full-write must preserve the successful alpha result in completed data",
        });
        assertions.push({
            name: "coverage:missing-beta",
            passed: betaCall !== undefined &&
                betaCall.status === "null" &&
                runtime.failures.some(({ callIndex }) => callIndex === betaCall.index) &&
                containsInNamedField(runtime.result, /miss|fail|error|unavailable/i, (value) => containsIdentity(value, "beta")),
    ...
    if (scenario.id === "coverage-fan-out-synthesize") {
        const contributorCalls = runtime.calls.filter(({ label, prompt }) => resolveExclusiveIdentity(label, prompt, ["climate", "transport"]) !== undefined);
        const synthesisCalls = runtime.calls.filter((call) => !contributorCalls.includes(call));
        const synthesisCall = synthesisCalls[0];
        const climateCall = contributorCalls.find(({ label, prompt }) => resolveExclusiveIdentity(label, prompt, ["climate", "transport"]) === "climate");
        const transportCall = contributorCalls.find(({ label, prompt }) => resolveExclusiveIdentity(label, prompt, ["climate", "transport"]) === "transport");
        assertions.push({
            name: "synthesis:complete-fan-out",
            passed: contributorCalls.length === 2 &&
            ...
            details: "fan-out-and-synthesize must run both structured contributors concurrently and preserve failure evidence",
        });
        assertions.push({
            name: "synthesis:complete-input",
            passed: synthesisCalls.length === 1 &&
            ...
            details: "synthesis must start from the complete intended coverage, including the climate result and missing transport identity",
        });
        assertions.push({
            name: "synthesis:returned-provenance",
            passed: synthesisCall !== undefined &&
                climateCall !== undefined &&
                containsValue(runtime.result, synthesisCall.result) &&
                containsInNamedField(runtime.result, /contribut|coverage|ledger/i, (value) => containsIdentity(value, "climate") &&
                    containsIdentity(value, "transport") &&
                    containsMissingCoverage(value, "transport")),
            details: "fan-out-and-synthesize must return the actual synthesis and a contributor ledger with missing transport coverage",
        });
    }
    if (scenario.id === "coverage-generate-filter") {
        ...
        for (const candidate of generatedCandidates) {
            if (typeof candidate.id !== "string" || seenCandidateIds.has(candidate.id))
                continue;
            seenCandidateIds.add(candidate.id);
            if (retainedCandidates.length < 3)
                retainedCandidates.push(candidate);
        }
        const filterCallsById = new Map(retainedCandidates.map((candidate) => [
            candidate.id,
            filterCalls.filter(({ prompt }) => promptContainsCandidate(prompt, candidate)),
        ]));
        assertions.push({
            name: "filter:generation-barrier",
            passed: generatorCalls.length === 2 &&
                generatorCalls.every(({ structured }) => structured) &&
                runtime.topology.maxConcurrent > 1 &&
                generatorCalls.every(({ completedIndex }) => completedIndex !== null) &&
                filterCalls.every(({ index }) => index > Math.max(...generatorCalls.map(({ completedIndex }) => completedIndex ?? Number.POSITIVE_INFINITY))),
        ...
        assertions.push({
            ...
            passed: retainedCandidates.length > 0 &&
            ...
            details: "generate-and-filter must deduplicate by candidate id, cap at three, and filter each retained candidate once",
        });
        ...
        const generatorOutputsReturned = containsInNamedField(runtime.result, /generat|output|batch/i, (value) => generatedCandidates.every((candidate) => containsCandidateProjection(value, candidate)));
        assertions.push({
            name: "filter:returned-provenance",
            passed: generatorOutputsReturned &&
                retainedCandidates.every(({ id }) => typeof id === "string" &&
                    containsInNamedField(runtime.result, /retain|candidate.?id/i, (value) => containsIdentity(value, id))) &&
                acceptedCalls.every((call) => {
                    const identity = resolveGenerateFilterIdentity(call.label, call.prompt, retainedCandidates.flatMap(({ id }) => (typeof id === "string" ? [id] : [])));
                    return (identity !== undefined &&
                        containsInNamedField(runtime.result, /accept|pass|selected/i, (value) => containsIdentity(value, identity)) &&
            ...
            details: "generate-and-filter must return actual generation, retained identities, accepted decisions, and failed filter identity",
        });
    }
    if (scenario.id === "coverage-judge-panel") {
        const qualityStart = runtime.events.find((event) => event.type === "quality" && event.stage === "start" && event.helper === "judgePanel");
        const qualityEnd = runtime.events.find((event) => event.type === "quality" && event.stage === "end" && event.helper === "judgePanel");
        const judgmentCalls = qualityStart && qualityEnd
        ...
        const judgedAttempts = candidateCalls.map((candidate, index) => judgedAttemptFromPrompt(judgmentCalls.find(({ label }) => label.startsWith(`judge ${index + 1}.`))?.prompt ?? "", candidate.result));
        const judgmentsByCandidate = candidateCalls.map((_candidate, index) => judgmentCalls.filter(({ label }) => label.startsWith(`judge ${index + 1}.`)));
        assertions.push({
            name: "judge-panel:executed",
            passed: qualityStart !== undefined &&
                qualityEnd !== undefined &&
                candidateCalls.length === 2 &&
                candidateCalls.every(({ structured }) => structured) &&
                runtime.topology.maxConcurrent > 1,
            details: "judge-panel scenario must produce exactly two structured candidates concurrently and execute judgePanel",
        });
        assertions.push({
            name: "judge-panel:numeric-judges",
            passed: judgmentCalls.length === 6 &&
                judgmentCalls.every(({ structured, prompt }) => structured && /correct|valid|accur/i.test(prompt)) &&
                judgmentsByCandidate.every((calls, index) => calls.length === 3 &&
            ...
            details: "judgePanel must run exactly three structured correctness judges per produced candidate and retain one recoverable failure",
        });
        const scoredCandidates = judgmentsByCandidate.map((calls, index) => {
            const surviving = calls.filter(({ status }) => status === "returned");
            const scores = surviving.map(({ result }) => isRecord(result) && typeof result.score === "number" ? result.score : 0);
            return {
                index,
        ...
        const expectedWinner = scoredCandidates.reduce((best, candidate) => !best || candidate.score > best.score || (candidate.score === best.score && candidate.index < best.index)
            ? candidate
            : best, undefined);
        assertions.push({
            name: "judge-panel:returned-winner",
            passed: expectedWinner !== undefined &&
    ...
    if (scenario.id === "full-edit") {
        const childStarts = runtime.events.filter((event) => event.type === "workflow" && event.stage === "start");
        const childEnds = runtime.events.filter((event) => event.type === "workflow" && event.stage === "end");
        ...
        assertions.push({
            name: "phase:entered",
            passed: preparation !== undefined &&
                runtime.events.some((event) => event.type === "phase" && event.budget !== null && event.budget > 0 && event.index < preparation.index),
    ...
    if (scenario.id === "full-review") {
        ...
        const qualityCalls = qualityStart && qualityEnd
            ? runtime.calls.filter(({ index }) => qualityStart.index < index && index < qualityEnd.index)
            : [];
    ...
    if (scenario.id === "full-debug") {
        ...
        assertions.push({
            ...
            passed: firstAttemptCall?.structured === true &&
    ...
    if (scenario.id === "full-loop") {
        ...
        const firstFinding = isRecord(firstCall?.result) && Array.isArray(firstCall.result.findings)
            ? firstCall.result.findings[0]
            : undefined;
        assertions.push({
            name: "loop:successful-dry-stopping",
            passed: runtime.calls.length === 4 &&
        ...
        assertions.push({
            ...
            passed: firstFinding !== undefined &&
                containsValue(runtime.result, firstFinding) &&
                failedCall !== undefined &&
                containsInNamedField(runtime.result, /fail|miss|error/i, (value) => containsRoundIdentity(value, failedCall.label, runtime.calls.indexOf(failedCall))),
        ...
        assertions.push({
            ...
            passed: containsInNamedField(runtime.result, /complete/i, (value) => value === false) &&
    ...
    if (scenario.id === "full-retry") {
        const controlAttempts = runtime.events.filter((event) => event.type === "control-attempt" && event.helper === "retry");
        const firstCall = runtime.calls[0];
        const acceptedCall = runtime.calls[1];
        assertions.push({
            name: "retry:exact-control-contract",
            passed: runtime.calls.length === 2 &&
        ...
        assertions.push({
            ...
            passed: firstCall !== undefined &&
                acceptedCall !== undefined &&
                containsInNamedField(runtime.result, /attempt|ledger|history/i, (value) => containsResultProjection(value, firstCall.result) && containsResultProjection(value, acceptedCall.result)) &&
...
function sampleSchema(schema, label, taskIdentity) {
    if (isRecord(schema) && schema.type === "object") {
        const properties = isRecord(schema.properties) ? schema.properties : {};
        return Object.fromEntries(Object.entries(properties).map(([key, value]) => [key, sampleProperty(key, value, label, taskIdentity)]));
    }
    return sampleProperty("value", schema, label, taskIdentity);
}
function sampleProperty(key, schema, label, taskIdentity) {
...
function containsIdentity(container, identity) {
    ...
    return Object.entries(container).some(([key, value]) => key.toLowerCase() === identity.toLowerCase() || containsIdentity(value, identity));
}
function containsRoundIdentity(container, label, zeroBasedIndex) {
    ...
    return (isRecord(container) &&
        Object.entries(container).some(([key, value]) => containsRoundIdentity(key, label, zeroBasedIndex) || containsRoundIdentity(value, label, zeroBasedIndex)));
}
function containsTaskResult(container, workId, expected) {
    if (Array.isArray(container)) {
    ...
    if (values.some((value) => typeof value === "string" && value.toLowerCase() === workId.toLowerCase()) &&
        values.some((value) => containsValue(value, expected))) {
        return true;
    }
    return values.some((value) => containsTaskResult(value, workId, expected));
}
function containsTaskResultEvidence(container, identity, expected) {
    ...
    if (isRecord(expected) && containsIdentity(container, identity)) {
        const producedPayload = Object.entries(expected).filter(([, value]) => typeof value !== "boolean" &&
...
function containsNamedResultProjection(container, expected, keys) {
    if (Array.isArray(container)) {
        return container.some((value) => containsNamedResultProjection(value, expected, keys));
    }
    if (!isRecord(container) || !isRecord(expected))
        return false;
    if (keys.every((key) => Object.hasOwn(expected, key) &&
...
function containsResultProjection(container, expected) {
    if (isDeepStrictEqual(container, expected))
        return true;
    if (Array.isArray(container)) {
...
function promptContainsValue(prompt, value) {
...
function isDecisionBooleanKey(key) {
    return /^(?:accept|accepted|pass|passed|selected)$/i.test(key);
}
function isDecisionExplanationKey(key) {
    return /^(?:reason|rationale|explanation)$/i.test(key);
}
function hasDecisionProperties(properties) {
    return Object.keys(properties).some((key) => isDecisionBooleanKey(key) || isDecisionExplanationKey(key) || /^(?:decision|verdict|score)$/i.test(key));
}
function schemaLiteralOr(schema, fallback) {
    if (isRecord(schema) && Object.hasOwn(schema, "const")) {
        return schema.const;
    }
    if (isRecord(schema) && Array.isArray(schema.enum) && schema.enum.length > 0) {
...
function isCandidateSchema(schema) {
    if (!isRecord(schema) || schema.type !== "object") {
        return false;
    }
    const properties = isRecord(schema.properties) ? schema.properties : {};
...
function schemaContainsCandidate(schema) {
    ...
    if ((Object.hasOwn(schema, "const") && generatedCandidatesFromResult(schema.const) !== null) ||
        (Array.isArray(schema.enum) && generatedCandidatesFromResult(schema.enum) !== null)) {
        return true;
    }
    if (schema.type === "array") {
        const prefixItems = Array.isArray(schema.prefixItems) ? schema.prefixItems : [];
        return (prefixItems.some((itemSchema) => schemaContainsCandidate(itemSchema)) ||
...
function isGenerateFilterGeneratorSchema(schema) {
    if (!isRecord(schema)) {
        return false;
    }
    const properties = isRecord(schema.properties) ? schema.properties : {};
...
function candidateFromSchema(schema, candidate, label) {
...
function fillGeneratorSchema(schema, candidates, label) {
    if (isRecord(schema) && Object.hasOwn(schema, "const")) {
        return {
            value: schema.const,
    ...
    if (schema.type === "array") {
        ...
        const enumeratedItems = isRecord(schema.items) && Array.isArray(schema.items.enum)
            ? generatedCandidatesFromResult(schema.items.enum)
            : null;
        if (enumeratedItems) {
            const maxItems = typeof schema.maxItems === "number" && Number.isFinite(schema.maxItems)
...
function createGeneratorResult(schema, candidates, label) {
    if (isCandidateSchema(schema)) {
...
function generatedCandidatesFromResult(result) {
    const candidates = [];
    const collect = (value) => {
...
function fillFilterDecisionSchema(schema, identity, accepted, label) {
    if (!isRecord(schema) || schema.type !== "object") {
...
function createFilterDecisionResult(schema, identity, accepted) {
...
function filterDecisionIsAccepted(result) {
    ...
    return Object.entries(result).some(([key, value]) => (isDecisionBooleanKey(key) && value === true) ||
        ((!isDecisionBooleanKey(key) || value !== false) && filterDecisionIsAccepted(value)));
}
function containsCandidateProjection(container, candidate) {
    return containsNamedResultProjection(container, candidate, ["id", "proposal"]);
}
function decisionPayloadValues(result) {
    if (Array.isArray(result)) {
...
function containsFilterDecisionEvidence(container, result, identity) {
...
function singleFieldProjection(value) {
    if (!isRecord(value))
        return [];
...
function promptContainsJsonProjection(prompt, result) {
    return promptContainsValue(prompt, result) || jsonValuesInText(prompt).some((value) => containsValue(value, result));
}
function promptContainsResultOrSingleFieldProjection(prompt, result) {
    return (promptContainsJsonProjection(prompt, result) ||
        singleFieldProjection(result).some((value) => promptContainsJsonProjection(prompt, value)));
}
function containsMissingCoverage(container, identity) {
    if (Array.isArray(container)) {
...
function judgedAttemptFromPrompt(prompt, candidateResult) {
    return jsonValuesInText(prompt).find((value) => containsResultProjection(value, candidateResult));
}
function containsResultOrSingleFieldProjection(container, result) {
    return (containsValue(container, result) || singleFieldProjection(result).some((value) => containsValue(container, value)));
}
